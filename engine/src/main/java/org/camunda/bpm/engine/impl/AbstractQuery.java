...
public abstract class AbstractQuery<T extends Query<?,?>, U> extends ListQueryParameterObject implements Command<Object>, Query<T,U>, Serializable {
  ...
  @SuppressWarnings("unchecked")
  public List<ComparablePair<String, String>> listDeploymentIdMappings() {
    this.resultType = ResultType.LIST_DEPLOYMENT_ID_MAPPINGS;
    List<ComparablePair<String, String>> ids = null;
    if (commandExecutor != null) {
      ids = (List<ComparablePair<String, String>>) commandExecutor.execute(this);
    } else {
  ...
  public List<ComparablePair<String, String>> evaluateExpressionsAndExecuteDeploymentIdMappingsList(CommandContext commandContext) {
    validate();
    evaluateExpressions();
    return !hasExcludingConditions() ? executeDeploymentIdMappingsList(commandContext) : new ArrayList<>();
  }

  public List<ComparablePair<String, String>> executeDeploymentIdMappingsList(CommandContext commandContext) {
    throw new UnsupportedOperationException();
  }
